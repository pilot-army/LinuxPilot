...
2. Apply schema: `pnpm db:migrate:deploy` or start the `auth-migrate` Compose service.
...
5. Start Auth Service, then API Gateway, then the web UI.
...
2. Set `SERVICE_AUTH_SECRET_PREVIOUS` to the current secret on Auth Service.
