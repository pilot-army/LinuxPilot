...
- [ ] Do not publish `api-gateway` or `auth-service` ports to the internet. Only the reverse proxy / web port should be public.
- [ ] Set `TRUST_PROXY=true` only for the hop count of your real reverse proxy.
- [ ] Set `FRONTEND_ORIGIN` to the public UI origin.
- [ ] Run `pnpm db:migrate:deploy` (or the `auth-migrate` Compose service) before starting Auth Service.
