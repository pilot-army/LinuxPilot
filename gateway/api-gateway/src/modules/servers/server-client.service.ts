...
export class ServerClientService {
  ...
  async request<T>(
    ...
    const timeout = setTimeout(() => controller.abort(), this.config.env.SERVER_SERVICE_TIMEOUT_MS);

    let response: Response;
    try {
      response = await fetch(`${this.config.env.SERVER_SERVICE_URL}${options.path}`, {
        method: options.method,
        headers,
        body: options.method === 'GET' ? undefined : serialized || undefined,
        signal: controller.signal,
      });
    } catch (error) {
      if (isAbortError(error)) {
        throw new AppError(AUTH_ERROR_CODES.INTERNAL_ERROR, 'Server service timed out', 504);
      }
      throw new AppError(AUTH_ERROR_CODES.INTERNAL_ERROR, 'Server service is unavailable', 502);
    } finally {
    ...
    if (!response.ok || 'error' in payload) {
      ...
      throw new AppError(
        ...
        sanitizePublicMessage(error?.message ?? 'Server service request failed'),
