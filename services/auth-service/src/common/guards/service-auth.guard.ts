...
import { NonceStore } from '../security/nonce-store';
...
export class ServiceAuthGuard implements CanActivate {
  ...
  constructor(private readonly config: AppConfigService) {
    this.nonces = new NonceStore(this.config.env.SERVICE_AUTH_MAX_SKEW_MS * 2);
  }
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!timestamp || !nonce || !signature) {
      throw new AppError(AUTH_ERROR_CODES.UNAUTHORIZED, 'Service authentication required', 401);
    }
